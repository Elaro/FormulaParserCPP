...
This library is intended to be used as part of a digital asset creation program (either as an internal element or as part of a plugin). It provides the ability for its user to use mathematical formulas to define a part, or the whole, of the asset's data points (such as notes in a MIDI file, vertex positions in a 3D model, bone position in an animation, etc). 
...
**Access to an indexable numeric data structure through the data[] operator**: A Formula can use numbers from any indexable data structure (with constant dimensions) as operands. Formula  
