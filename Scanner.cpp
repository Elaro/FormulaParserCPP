...
void Scanner::Init() {
	...
	start.set(43, 19);
	start.set(45, 20);
	for (i = 37; i <= 37; ++i) start.set(i, 21);
	for (i = 42; i <= 42; ++i) start.set(i, 21);
	for (i = 47; i <= 47; ++i) start.set(i, 21);
	start.set(94, 22);
	start.set(44, 23);
	start.set(40, 24);
	start.set(41, 25);
	...
	keywords.set(L"cos", 4);
	keywords.set(L"tan", 5);
	keywords.set(L"asin", 6);
	keywords.set(L"acos", 7);
	keywords.set(L"atan", 8);
	keywords.set(L"sinh", 9);
	keywords.set(L"cosh", 10);
	keywords.set(L"tanh", 11);
	keywords.set(L"log", 12);
	keywords.set(L"ln", 13);
	keywords.set(L"sqrt", 14);
	keywords.set(L"ceil", 15);
	keywords.set(L"floor", 16);
	keywords.set(L"abs", 17);
	keywords.set(L"sum", 18);
	keywords.set(L"mult", 19);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 9:
			case_9:
			if ((ch >= L'0' && ch <= L'7')) {AddCh(); goto case_10;}
			else {goto case_0;}
		case 10:
			case_10:
			recEnd = pos; recKind = 20;
		...
		case 11:
			case_11:
			if ((ch >= L'0' && ch <= L'7')) {AddCh(); goto case_12;}
			else {goto case_0;}
		case 12:
			case_12:
			recEnd = pos; recKind = 20;
		...
		case 13:
			case_13:
			if ((ch >= L'0' && ch <= L'1')) {AddCh(); goto case_14;}
			else {goto case_0;}
		case 14:
			case_14:
			recEnd = pos; recKind = 20;
		...
		case 15:
			case_15:
			if ((ch >= L'0' && ch <= L'1')) {AddCh(); goto case_16;}
			else {goto case_0;}
		case 16:
			case_16:
			recEnd = pos; recKind = 20;
		...
		case 28:
			...
			else if (ch == L'b') {AddCh(); goto case_13;}
			else {t->kind = 20; break;}
